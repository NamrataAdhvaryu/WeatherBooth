...
struct PresentableWeatherData: Identifiable {
    let dailyWeather: [PresentableDailyy]
    let currenWeather: Currentt
    ...
    init(weather: WeatherData) {
        var dailyArray = [PresentableDailyy]()
        for daily in weather.daily {
            dailyArray.append(PresentableDailyy(daily: daily))
        }
...
struct PresentableDailyy: Identifiable {
    let date: Date
    let minTemp: Double
    let maxTemp: Double
